...
public class ControlActions extends JPanel implements ActionListener {
	...
	DefaultListModel<CAData> listModel;
	CAData myData;

	public ControlActions() {

		// リストの初期化
		listModel = AnalysisData.getInstance().getCAData();
		...
		add(new JLabel("Please input Control Actions title and note."), new TMGridBagConstraints(0, 1, 2, 1));
		...
		add(new JLabel("CA list"), new TMGridBagConstraints(0, 6, 2, 1));
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (actionCommand.equals("AddButton")) {
			...
			if (titleField.getText().length() > 0) {
				myData = new CAData();
				myData.setId("CA-" + (listModel.size() + 1));
				...
				// データを同期させる
				analysisData.setCAData(listModel);
			}
		}
		...
		if(actionCommand.equals("DeleteButton")) {
			if(list.getSelectedIndex() >= 0) {
				...
				for(int i= list.getSelectedIndex(); i < listModel.size();i++) {
					listModel.getElementAt(i).setId("CA-" + i);
				}
