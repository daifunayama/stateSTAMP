...
public class SystemHazards extends JPanel implements ActionListener {
	...
	public SystemHazards() {

		// リストの初期化
		listModel = AnalysisData.getInstance().getHazardData();
		...
		add(new JLabel("Please input System Hazard title and note."), new TMGridBagConstraints(0, 1, 2, 1));
		...
		add(new JLabel("Hazard list"), new TMGridBagConstraints(0, 6, 2, 1));
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (actionCommand.equals("AddButton")) {
			...
			if (titleField.getText().length() > 0) {
				...
				myData.setId("H-" + (listModel.size() + 1));
				...
				// データを同期させる
				analysisData.setHazardData(listModel);
			}
		}
		...
		if(actionCommand.equals("DeleteButton")) {
			if(list.getSelectedIndex() >= 0) {
				...
				for(int i= list.getSelectedIndex(); i < listModel.size();i++) {
					listModel.getElementAt(i).setId("H-" + i);
				}
